...
# 5365
# ]
#begin_end_swap = [('2021-06-08 13:00', '2021-07-01')]
#begin_end_swap = [(None, None), (None, None)]
# all_filter_dates = {
#     # Council Crest (Snow February 11-15 2021 Melt through 20th? )
#                 13838:
#                 [('2021-01-26 14:00', '2021-01-30 08:00'),
...
location_ids = [6183, 6186, 13703, 13815, 6184, 13820, 13821, 13814, 4771, 13835, 13822]

#location_ids =[13814]

output_hdf5 = r"V:\HydroModHSPF\Models\HSPFWorkshopMaterials\HSPFData\ObservedFlow"

combined_location_id =
combined_temp_monitor_filtered_flow_dfs = []
combined_temp_monitor_raw_flow_dfs = []

for location_id in location_ids:
        #begin_swap = begin_end_swap[0]
        #end_swap = begin_end_swap[1]
        print(location_id)
        ...
        if all_filter_dates[location_id] is not None:
            ...
            beg_date, end_date = filter_dates[0]
            temp_monitor_filtered.raw_data = temp_monitor1.raw_data.loc[beg_date: end_date].copy()
            for beg_date, end_date in filter_dates[1:]:
                print(beg_date)
                print(end_date)
                test = temp_monitor1.raw_data.loc[beg_date: end_date].copy()
            ...
            print("no dates")
        monitors_to_combine.append(temp_monitor_filtered)

combined_temp_monitor_filtered.flow_data.dropna().to_hdf(output_hdf5 + "\\" + "raw" + str(combined_location_id) + ".h5", mode='w', key='df')
combined_temp_monitor_filtered.filtered_flow_data.dropna().to_hdf(output_hdf5 + "\\" + "filtered" + str(combined_location_id) + ".h5", mode='w', key='df')
try:
    combined_temp_monitor_filtered.flow_data.dropna().to_excel(output_hdf5 + "\\" + "raw" + str(combined_location_id) + ".xlsx")
except:
    print("raw data too large for excel")
try:
    combined_temp_monitor_filtered.filtered_flow_data.dropna().to_excel(output_hdf5 + "\\" + "filtered" + str(combined_location_id) + ".xlsx")
except:
